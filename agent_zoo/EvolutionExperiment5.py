...
MUTATION_RATE = .25
CLONE_RATE = .2
MAX_FRAME = 100  # how many frames is the robot simulated for
    ...
    def get_weights(self):
        ...
        for layer in self.svd_dic:
            if self.svd_dic[layer][0] is True:
                ...
                weights[layer] = copy.deepcopy(self.svd_dic[layer][1])
            i += 1
        return weights
...
def main():
    ...
    with open('resultsb.csv', 'w') as writer_results:
        with open('Elite_Individual_Experiment5b.weights', 'w') as wwriter:
